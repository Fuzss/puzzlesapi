...
public class ClientInputActionHandler {
    ...
    public static EventResult onBeforeMouseScroll(AbstractContainerScreen<?> screen, double mouseX, double mouseY, double horizontalAmount, double verticalAmount) {
        ...
        if (!ItemInteractionsCore.CONFIG.get(ClientConfig.class).revealContents.isActive()) return EventResult.PASS;
        if (precisionModeAllowedAndActive()) {
            ...
            if (slot != null) {
                if (ItemContainerProviders.INSTANCE.get(screen.getMenu().getCarried()) != null || ItemContainerProviders.INSTANCE.get(slot.getItem()) != null) {
                    int mouseButton = (ItemInteractionsCore.CONFIG.get(ClientConfig.class).invertPrecisionModeScrolling ? verticalAmount < 0.0 : verticalAmount > 0.0) ? InputConstants.MOUSE_BUTTON_RIGHT : InputConstants.MOUSE_BUTTON_LEFT;
        ...
        } else if (ItemInteractionsCore.CONFIG.get(ServerConfig.class).allowSlotCycling) {
            ItemStack carriedStack = screen.getMenu().getCarried();
            if (!carriedStack.isEmpty() && !ItemInteractionsCore.CONFIG.get(ClientConfig.class).carriedItemTooltips.isActive()) {
                return EventResult.PASS;
            }
    ...
    public static EventResult onPlaySoundAtPosition(Level level, Entity entity, MutableValue<Holder<SoundEvent>> sound, MutableValue<SoundSource> source, DefaultedFloat volume, DefaultedFloat pitch) {
        if (!ItemInteractionsCore.CONFIG.get(ClientConfig.class).disableInteractionSounds) return EventResult.PASS;
    ...
    public static boolean precisionModeAllowedAndActive() {
        return ItemInteractionsCore.CONFIG.get(ServerConfig.class).allowPrecisionMode && ItemInteractionsCore.CONFIG.get(ClientConfig.class).precisionMode.isActive();
    }
    ...
    public static void ensureHasSentContainerClientInput(Screen screen, Player player) {
        ...
        if (currentContainerSlot != lastSentContainerSlot || extractSingleItem != lastSentExtractSingleItem) {
            ...
            ItemInteractionsCore.NETWORK.sendToServer(new C2SContainerClientInputMessage(currentContainerSlot, extractSingleItem));
        }
    }
}
