...
import fuzs.puzzlesapi.mixin.statues.accessor.SimpleContainerAccessor;
...
public class ArmorStandMenu extends AbstractContainerMenu implements ArmorStandHolder {
    public static final ResourceLocation EMPTY_ARMOR_SLOT_SWORD = Statues.id("item/empty_armor_slot_sword");
    ...
    public static ArmorStandMenu create(MenuType<?> menuType, int containerId, Inventory inventory, FriendlyByteBuf buf, @Nullable ArmorStandDataProvider dataProvider) {
        ...
        if (entity != null) {
            ...
            ((ArmorStandAccessor) entity).puzzlesapi$setDisabledSlots(buf.readInt());
    ...
    public static ArmorStandMenu create(MenuType<?> menuType, int containerId, Inventory inventory, ArmorStand armorStand, @Nullable ArmorStandDataProvider dataProvider) {
        ...
        NonNullList<ItemStack> armorItems = ((ArmorStandAccessor) armorStand).puzzlesapi$getArmorItems();
        NonNullList<ItemStack> handItems = ((ArmorStandAccessor) armorStand).puzzlesapi$getHandItems();
    ...
    private static SimpleContainer simpleContainer(NonNullList<ItemStack> items) {
        ...
        ((SimpleContainerAccessor) container).puzzlesapi$setItems(items);
        return container;
    }
    ...
    public static boolean isSlotDisabled(ArmorStand armorStand, EquipmentSlot slot, int offset) {
        return (((ArmorStandAccessor) armorStand).puzzlesapi$getDisabledSlots() & 1 << slot.getFilterFlag() + offset) != 0;
    }
